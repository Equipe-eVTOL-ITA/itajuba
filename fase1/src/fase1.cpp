...
public:
    ...
    // DYNAMIC GRID ----------------------------------------------------
        ...
        while (num_steps_taken < num_steps || !finished) {
            ...
            state++;
            waypoints.push_back({Eigen::Vector3d({x_coord - (num_steps * step_x / 2), y_coord - (y_length / 2), takeoff_height})});
        }
...
public:
    NodeFSM(std::shared_ptr<Drone> drone, std::shared_ptr<VisionNode> vision) 
        ...
        fsm_ = std::make_unique<fase1FSM>(drone_node_, vision_node_, params, target_shapes);
